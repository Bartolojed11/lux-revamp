    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            const products = JSON.parse(
                localStorage.getItem('selected_products')
            )
            if (products !== null) {
        ...
        return (
            <div className="delivery-address">
                <div className="delivery-address__header">
                    <span className="font-600 vertical-center">
                        <IoLocationSharp className="font-red" />
                        Delivery Address
                    </span>
                    <Link
                        href="/my-address/select"
                        className="font-underline font-14"
                    >
                        Change Address
                    </Link>
                </div>
